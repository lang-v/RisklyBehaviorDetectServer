...
with tf.Graph().as_default():
    ...
    with tf.compat.v1.name_scope('loss'):
        loss = tf.reduce_mean(
            input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=tf.stop_gradient(batch_labels)))
        tf.compat.v1.summary.scalar('entropy_loss', loss)
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        for epoch in range(EPOCH_NUM):
            ...
            print('Epoch %d: Average loss is: %.5f; Average accuracy is: %.5f' % (
                epoch + 1, loss_epoch / (len(train_video_indices) // BATCH_SIZE),
