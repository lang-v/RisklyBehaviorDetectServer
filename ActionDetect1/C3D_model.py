...
def C3D(input, num_classes, keep_pro=0.5):
    with tf.compat.v1.variable_scope('C3D'):
        with slim.arg_scope([slim.conv3d],
            ...
            net = slim.fully_connected(net, 4096, weights_regularizer=tf.keras.regularizers.l2(0.5 * (0.0005)), scope='fc6')
            net = slim.dropout(net, keep_pro, scope='dropout1')
            net = slim.fully_connected(net, 4096, weights_regularizer=tf.keras.regularizers.l2(0.5 * (0.0005)), scope='fc7')
            net = slim.dropout(net, keep_pro, scope='dropout2')
            out = slim.fully_connected(net, num_classes, weights_regularizer=tf.keras.regularizers.l2(0.5 * (0.0005)),
